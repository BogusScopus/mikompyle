...
# ─────────────────────────────────────────────────────────────────────────────
# Circuit variable-count introspection
# ─────────────────────────────────────────────────────────────────────────────

def circuit_max_var(circuit) -> int:
    """Return the highest variable index (1-based) that appears as a leaf in
    the circuit.

    Leaf nodes are at layer 0.  Their indices encode literals:
      - index 0  → False constant
      - index 1  → True  constant
      - index 2v → positive literal of variable v (v ≥ 1)
      - index 2v+1 → negative literal of variable v

    So  max_var = max(leaf_indices filtered to > 1) // 2.

    If the ganak circuit's max_var differs from the original DIMACS nb_vars,
    Arjun has renumbered or eliminated variables and the weight vector passed
    by ``encode_log`` is mis-indexed.  See module docstring for the required
    C++ fix.
    """
    get_ix = getattr(circuit, "_get_indices", None) or getattr(circuit, "get_indices")
    ixs_in, _ = get_ix()
    if not ixs_in:
        return 0
    leaf_ixs = np.asarray(ixs_in[0])
    var_ixs = leaf_ixs[leaf_ixs > 1]
    return int(np.max(var_ixs)) // 2 if len(var_ixs) else 0


def circuit_all_vars(circuit) -> List[int]:
    """Return sorted list of all variable indices (1-based) present as leaves."""
    get_ix = getattr(circuit, "_get_indices", None) or getattr(circuit, "get_indices")
    ixs_in, _ = get_ix()
    if not ixs_in:
        return []
    leaf_ixs = np.asarray(ixs_in[0])
    var_ixs = leaf_ixs[leaf_ixs > 1]
    return sorted(set((int(i) // 2 for i in var_ixs)))
...
# ── 0. Variable-mapping diagnostic (exposes the Arjun renaming bug) ───────────

@requires_my_ext
class TestVariableMapping:
    """Diagnose whether ganak circuits preserve original DIMACS variable indices.

    Arjun (the CNF preprocessor inside compile_to_ganak) may:
      (a) Eliminate variables via BVE and renumber the survivors  →  circuit
          literal nodes use internal indices, not original DIMACS indices.
      (b) Keep all variables but reorder them  →  same mismatch.

    Both cases mean the weight vector built by ``encode_log(nb_vars, w, nw)``
    is mis-indexed: weight for original var k ends up at slot 2k, but the
    circuit reads that slot for internal var k (which may be a different
    original variable).

    These tests FAIL when the bug is present.  Once core.cpp is fixed (see
    module docstring), they should all pass.
    """

    @pytest.mark.parametrize("var_count,clause_count,seed", [
        (5, 15, 1),    # expected to pass (Arjun may not rename small formulas)
        (5, 15, 4),
        (8, 24, 1),
        (8, 24, 2),
        (8, 24, 3),
    ...
    def test_circuit_vars_match_dimacs(self, var_count, clause_count, seed):
        """The maximum variable index in the compiled circuit must equal the
        number of variables declared in the DIMACS header.

        Failure means Arjun renumbered variables: circuit leaf nodes are
        indexed by internal vars 1..M (M < nb_vars), but ``encode_log`` puts
        weights at positions for original vars 1..nb_vars.  The WMC will be
        wrong for any formula where M ≠ nb_vars.
        """
        path = make_temp_cnf(var_count, clause_count, seed)
        try:
            circuit = my_ext.compile_to_ganak(path)
            max_v   = circuit_max_var(circuit)
            all_v   = circuit_all_vars(circuit)

            assert max_v == var_count, (
                f"var={var_count} cls={clause_count} seed={seed}\n"
                f"  Circuit uses vars {all_v} (max={max_v}), "
                f"but DIMACS declares {var_count} variables.\n"
                f"  Arjun eliminated/renumbered {var_count - max_v} variable(s).\n"
                f"  encode_log({var_count}, w, nw) feeds wrong weights to the circuit.\n"
                f"  Fix: use cnf.get_orig_sampl_vars() in core.cpp to map internal "
                f"vars back to original DIMACS vars before creating literal nodes."
            )
        finally:
            os.unlink(path)

    def test_variable_mapping_diagnostic(self, capsys):
        """Print a table showing DIMACS nb_vars vs circuit max_var for a range
        of random CNFs.  Run with ``pytest -v -s`` to see the output.

        This is a *diagnostic* test that always passes; inspect the printed
        table to identify which sizes trigger Arjun renaming.
        """
        configs = [
            (5,  15, 1), (5,  15, 4),
        ...
        rows = []
        for vc, cc, seed in configs:
            path = make_temp_cnf(vc, cc, seed)
            try:
                circuit = my_ext.compile_to_ganak(path)
                max_v   = circuit_max_var(circuit)
                all_v   = circuit_all_vars(circuit)
                mismatch = (max_v != vc)
                rows.append((vc, cc, seed, max_v, all_v, mismatch))
            finally:
                os.unlink(path)

        print("\n\n  Variable mapping diagnostic")
        print(f"  {'vars':>5} {'cls':>5} {'seed':>4}  {'circ_max_var':>12}  "
              f"{'status':>10}  circuit vars")
        print("  " + "-" * 80)
        for vc, cc, seed, max_v, all_v, mismatch in rows:
            status = "*** BUG ***" if mismatch else "OK"
            print(f"  {vc:5d} {cc:5d} {seed:4d}  {max_v:12d}  {status:>10}  {all_v}")

        bugs = [(vc, cc, seed, max_v) for vc, cc, seed, max_v, _, m in rows if m]
        if bugs:
            print(f"\n  {len(bugs)} formula(s) have Arjun variable renaming:")
            for vc, cc, seed, max_v in bugs:
                print(f"    vars={vc} cls={cc} seed={seed}: "
                      f"expected max_var={vc}, got {max_v} "
                      f"(eliminated {vc-max_v} var(s))")
        else:
            print("\n  No renaming detected for these formula sizes.")
